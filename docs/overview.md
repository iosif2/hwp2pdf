...
| 문서 | 설명 |
|------|------|
| [overview.md](./overview.md) | 프로젝트 목적, 범위, 기술 스택, 리스크 요약 |
| [architecture.md](./architecture.md) | 시스템 구조, 레이어 역할, 변환 파이프라인 상세 |
| [api.md](./api.md) | API 엔드포인트 명세 및 요청/응답 예시 |
| [rhwp-notes.md](./rhwp-notes.md) | rhwp 조사 결과, 제약, 폰트 이슈, 리스크 |

---

## 1. 프로젝트 개요

HWP/HWPX 문서를 PDF로 변환하는 HTTP API 서버.  
Rust 기반 오픈소스 엔진인 rhwp를 변환 코어로 사용한다.

초기 단계에서는 **단일 노드에서 안정적으로 동작하는 동기식 변환 API** 구현에 집중한다.  
비동기 워커, 분산 처리는 이후 단계에서 점진적으로 도입한다.
...
- HTTP API 인터페이스 제공 (`POST /convert`, `GET /files/{id}.pdf`)
- 단일 요청 기준 낮은 레이턴시 변환 처리
- 내부 서비스에서 직접 호출 가능한 변환 엔드포인트 제공

### 비목표 (Out of Scope - 초기 단계)

- 대규모 비동기 워커 시스템 / 큐 기반 아키텍처
- 분산 처리
- 완벽한 렌더링 정합성 보장
- OCR 기반 fallback
